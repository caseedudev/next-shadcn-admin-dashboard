...
UI 페이지 설계 및 구현 요청 시 다음 6단계 워크플로를 따른다:
1. Step 0 - 프로젝트 분석 (.ui-designer/analysis.json)
2. Step 0.5 - 디자인 시스템 체크 (.ui-designer/design-system.md)
3. Step 1 - 요구사항 수집 (디자인 컨셉/컬러/폰트 추천 포함)
4. Step 2 - 디자인 제안 (스타일 시스템 포함)
5. Step 3 - 코드 생성 + 안티패턴 검사 + Vercel 검증
6. Step 4 - 디자인 시스템 저장

67개 디자인 컨셉, 116개 컬러 팔레트, 57개 폰트 페어링, 96개 산업별 추론 규칙을 기반으로
적극적인 추천/제안을 제공한다. `scripts/search.py`로 BM25 검색, 마크다운 references로 폴백.
...
| `/dashboard-template-init-project` | 도메인 용어집 + 프로젝트 개관 문서 생성 |
...
커맨드와 로컬 스킬이 프로젝트 루트의 `.codex/prompts/`, `.agents/skills/`에 설치된다.
글로벌 `~/.codex/auth.json`이 있으면 로컬 `.codex/auth.json`으로 자동 연결한다.
Codex는 이 프로젝트에서만 다음처럼 실행한다:
