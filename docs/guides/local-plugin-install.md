...
│       ├── agents/                 # 에이전트 (.md)
│       ├── hooks/                  # 훅 설정
│       ├── scripts/                # Codex/Antigravity용 실행 스크립트
│       └── install.sh              # 설치 스크립트
├── commands/                       # ← 심볼릭 링크 대상
...
- `scripts/*`가 있다면 `.agents/skills/{plugin-name}-{skill-name}/scripts/`로 함께 복사해 Codex 자동화에서 재사용한다
- 글로벌 `~/.codex/auth.json`이 있으면 로컬 `.codex/auth.json`으로 연결해 로그인 상태를 재사용한다
- 설치 후 Codex는 반드시 `CODEX_HOME="$PWD/.codex" codex`로 실행한다

### Codex 자동화 검증 권장 방식

Codex TTY/비대화형 환경에서는 슬래시 커맨드의 인터랙티브 Q&A가 불안정할 수 있다.
이 경우 플러그인 프롬프트만 믿지 말고 `scripts/`에 둔 실행 스크립트를 우선한다.
...
- 실제 셸로 재현 가능한 검증은 `scripts/`에 고정한다
- 인터랙티브 단계는 프롬프트 계약 검증이나 수동 세션으로 분리한다
- `install-codex.sh`는 프롬프트/스킬뿐 아니라 검증에 필요한 `scripts/`까지 함께 배포한다

### install-codex.sh 후처리 권장 사항

플러그인에 환경별 경로 차이가 있으면 `install-codex.sh`에서 복사 후 후처리를 수행하는 편이 안전하다.

대표 예시:

- 프롬프트 frontmatter의 `name:` 재작성
- Claude 전용 `model:`, `color:` 제거
- `.claude/plugins/local/...` 경로를 `.agents/skills/...` 경로로 치환
- `scripts/search.py` 같은 상대경로를 Codex 설치 대상 경로로 치환
- 복사된 스킬 디렉토리 안의 심볼릭 링크 제거

즉, 단순 `cp -R`만으로 끝내지 말고 `rewrite_prompt`, `rewrite_codex_file`, `sanitize_copied_skill` 같은 후처리 단계를 고려해야 한다.

## Antigravity 로컬 설치

Antigravity는 프로젝트 로컬 `.github/skills/`를 읽는 방식을 기준으로 설치한다.

### install-antigravity.sh 동작 원칙

- `skills/*/`는 `.github/skills/{plugin-name}-{skill-name}/`로 실제 복사한다
- Antigravity는 심볼릭 링크 보장이 약하므로 `cp -R`를 기본으로 한다
- `data/*`, `scripts/*`가 있으면 스킬 디렉토리에 함께 복사한다
- 필요하면 `.agents/skills/...`, `.claude/plugins/local/...` 참조를 `.github/skills/...` 경로로 치환한다
- 이전 버전 스킬 디렉토리가 남아 있으면 설치 전에 정리한다

### Antigravity 설치/검증 예시

```bash
bash .claude/plugins/local/{plugin-name}/install-antigravity.sh

find .github/skills -maxdepth 1 -type d -name '{plugin-name}-*'
```

### install-antigravity.sh 후처리 권장 사항

Antigravity도 Codex와 비슷하게 환경별 경로 차이가 생기므로 복사 후 후처리를 권장한다.

대표 예시:

- `.agents/skills/...` 경로를 `.github/skills/...` 경로로 치환
- `.claude/plugins/local/...` 경로를 `.github/skills/...` 경로로 치환
- `scripts/search.py` 같은 실행 경로를 `.github/skills/.../scripts/search.py`로 치환
- 복사된 스킬 내부 심볼릭 링크 제거
