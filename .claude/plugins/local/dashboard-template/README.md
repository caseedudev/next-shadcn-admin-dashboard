...
- `commands/dashboard-template/` — 슬래시 커맨드 (7개)
- `skills/dashboard-template-*` — 자동 트리거 스킬 (4개)
- `agents/` — 서브 에이전트 (3개)
...
프로젝트 로컬 `.codex/prompts/`에 커맨드 7개, `.agents/skills/`에 스킬 4개를 설치한다.
...
```bash
rm -rf .claude/commands/dashboard-template
rm -f .claude/skills/dashboard-template-*
rm -f .claude/agents/{backend-architect,frontend-architect,fullstack-dev}.md
rm -f .codex/prompts/dashboard-template-*.md
rm -rf .agents/skills/dashboard-template-*
```
...
| 스킬 | 트리거 조건 | 적용 규칙 |
|------|------------|-----------|
| `dashboard-template-frontend-dev` | React 컴포넌트, Next.js 페이지, UI 코드 작성/수정 | PERF, COMP, UI, STATE, UTIL, DS |
| `dashboard-template-backend-dev` | API 라우트, Supabase, SQL, 마이그레이션 작업 | API, SB, DATA, OBS, QA |
| `dashboard-template-fullstack-review` | PR 리뷰, 코드 리뷰, "review" 키워드 사용 | 전체 규칙 통합 적용 |
| `dashboard-template-project-init` | 프로젝트 초기화 시 또는 `docs/domain/glossary.md`·`project.md` 부재 감지 시 | 도메인 용어집 + 프로젝트 개관 생성 |

**작동 원리**: `src/app/api/` 하위 파일을 수정하면 `dashboard-template-backend-dev` 스킬이 자동 활성화되어 API-001(버저닝), API-002(zod 검증) 등의 규칙을 Claude가 인식하고 적용한다.
...
| 명령 | 설명 |
|------|------|
| `/dashboard-template-checklist` | 현재 git 변경사항에 대해 AI 체크리스트 실행 |
| `/dashboard-template-review-frontend` | 프론트엔드 코드 리뷰 |
| `/dashboard-template-review-backend` | 백엔드 코드 리뷰 |
| `/dashboard-template-new-api <domain>/<resource>` | `/api/v1/` 하위에 API 라우트 스캐폴딩 |
| `/dashboard-template-new-feature <domain>` | `src/features/` 하위에 피처 디렉토리 스캐폴딩 |
| `/dashboard-template-new-migration <description>` | Supabase 마이그레이션 파일 스캐폴딩 |
| `/dashboard-template-init-project` | 프로젝트 초기화 — 도메인 용어집(glossary.md) + 프로젝트 개관(project.md) 생성 |
...
Claude Code에서는 `frontend-architect` 에이전트를 생성하고, Codex에서는 로컬 프롬프트가 동일 규칙으로 변경된 프론트엔드 파일을 검사한다:
...
Claude Code에서는 `backend-architect` 에이전트가, Codex에서는 로컬 프롬프트가 API 라우트와 SQL 파일을 같은 규칙으로 검사한다:
...
├── skills/
│   ├── frontend-dev/                  # dashboard-template-frontend-dev
...
│   ├── backend-dev/                   # dashboard-template-backend-dev
│   │   ├── SKILL.md                   # 백엔드 개발 스킬
│   │   └── references/
│   │       ├── api-rules.md           # API-001~004 규칙
│   │       ├── supabase-rules.md      # SB-001~004, DATA-001~006 규칙
│   │       └── migration-rules.md     # 마이그레이션/시드 규칙
│   ├── fullstack-review/              # dashboard-template-fullstack-review
│   │   ├── SKILL.md                   # 풀스택 리뷰 스킬
│   │   └── references/
│   │       └── checklist.md           # AI 작업 체크리스트 통합본
│   └── project-init/                  # dashboard-template-project-init
...
├── commands/
│   ├── checklist.md                   # /dashboard-template-checklist
│   ├── review-frontend.md             # /dashboard-template-review-frontend
│   ├── review-backend.md              # /dashboard-template-review-backend
│   ├── new-api.md                     # /dashboard-template-new-api
│   ├── new-feature.md                 # /dashboard-template-new-feature
│   ├── new-migration.md               # /dashboard-template-new-migration
│   └── init-project.md                # /dashboard-template-init-project
├── hooks/
