...
run_search_suite() {
  ...
  if python3 "$SEARCH_SCRIPT" "minimalism clean" --domain style >/tmp/ui-qa-style.out 2>/tmp/ui-qa-style.err && rg -q 'score:' /tmp/ui-qa-style.out; then
    pass "1.1 도메인별 스타일 검색"
  else
    fail "1.1 도메인별 스타일 검색"
  fi

  if python3 "$SEARCH_SCRIPT" "fintech banking" >/tmp/ui-qa-industry.out 2>/tmp/ui-qa-industry.err && rg -q 'Industry Inference|Recommended matches' /tmp/ui-qa-industry.out; then
    pass "1.2 산업별 추론"
  else
    fail "1.2 산업별 추론"
  fi

  if python3 "$SEARCH_SCRIPT" "saas dashboard modern" --design-system >/tmp/ui-qa-ds.out 2>/tmp/ui-qa-ds.err && rg -q 'Style:|Colors:|Fonts:|Rules:' /tmp/ui-qa-ds.out; then
    pass "1.3 디자인 시스템 생성"
  else
    fail "1.3 디자인 시스템 생성"
  fi

  if python3 "$SEARCH_SCRIPT" "luxury fashion ecommerce" >/tmp/ui-qa-global.out 2>/tmp/ui-qa-global.err && rg -q 'Global Search:' /tmp/ui-qa-global.out; then
    pass "1.4 전체 도메인 검색"
  else
    fail "1.4 전체 도메인 검색"
  fi

  if python3 "$SEARCH_SCRIPT" "교육 앱 밝은" >/tmp/ui-qa-ko.out 2>/tmp/ui-qa-ko.err; then
    pass "1.5 한국어 검색"
  else
    fail "1.5 한국어 검색"
  fi
